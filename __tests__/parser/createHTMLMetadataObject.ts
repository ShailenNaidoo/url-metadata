import { createHTMLMetadataObject, createHTMLInstance } from '../../parser';
...
describe('createHTMLMetadataObject', (): void => {
  test('should return object with properties [title] and [og]', (): void => {
    const metadataObject = createHTMLMetadataObject(html);
  ...
  test('should return object that contains specific keys', (): void => {
    const { og } = createHTMLMetadataObject(html);
