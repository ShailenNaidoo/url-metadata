import { createHTMLInstance, filterOpenGraphTags, mapOpenGraphTags } from '../../parser';

const KEYS = ['site_name', 'type', 'image', 'title', 'description', 'url'];
...
const openGraphTags = metaTags.filter(filterOpenGraphTags);

describe('mapOpenGraphTags', (): void => {
  test('should be array of length 6', (): void => {
    expect(openGraphTags.map(mapOpenGraphTags).length).toBe(6);
  });

  test('should contain an array of objects that match KEYS', (): void => {
    expect(openGraphTags.map(mapOpenGraphTags).every((tag): boolean => KEYS.includes(Object.keys(tag)[0]))).toBe(true);
  });
});
